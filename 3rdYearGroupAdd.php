...
$req_fields = array('grpNo','title','coursecode','startDate','endDate','noOfStudents');
...
if(empty($errors)){
 $GroupNo =mysqli_real_escape_string($connection,$_POST['grpNo']);   
  ...
  $query = "INSERT INTO 3rd_year_group_project ( ";
  $query .=  "EmployeeID,GroupNo,CourseCode,ProjectTitle,StartDate,EndDate,noOfStudents,EvolutionTime,Academic_year,is_deleted";
  $query .= ") VALUES (";
  $query .= "'{$_SESSION['EmployeeID']}','{$GroupNo}','{$CourseCode}','{$ProjectTitle}','{$StartDate}','{$EndDate}','{$noOfStudents}','{$EvolutionTime}','{$_SESSION['Academic_year']}',0";
  $query .= ")";

  $result = mysqli_query($connection,$query);

  if($result){
    header('Location: 3rdYearProjectsView.php?user_added=true');
  }else{
...
<head>
	...
	<title>Third Year Project Add New</title>
...
?>
  <form action="3rdYearGroupAdd.php" method="post">
