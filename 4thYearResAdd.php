...
if(empty($errors)){
 $RegNo =mysqli_real_escape_string($connection,$_POST['stRegNo']);   
  ...
  $query = "INSERT INTO 4th_year_research ( ";
  $query .=  "EmployeeID,RegNo,StudentName,CourseCode,ResearchTitle,StartDate,EndDate,EvolutionTime,Academic_year,is_deleted";
  $query .= ") VALUES (";
  $query .= "'{$_SESSION['EmployeeID']}','{$RegNo}','{$StudentName}','{$CourseCode}','{$ResearchTitle}','{$StartDate}','{$EndDate}','{$EvolutionTime}','{$_SESSION['Academic_year']}',0";
  $query .= ")";

  $result = mysqli_query($connection,$query);

  if($result){
    header('Location: 4thYearResView.php?record_added=true');
  }else{
...
<head>
	...
	<title>4th Year Research Add New</title>
...
?>
  <form action="4thYearResAdd.php" method="post">
